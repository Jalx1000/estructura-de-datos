...
direccion_mem CSMemoriakeor::new_espacio(string cadena){
...
void CSMemoriakeor::delete_espacio(direccion_mem dir) {
	int x = dir;
	while(mem[x].link !=-1){
		x = mem[x].link;
	}
...
void CSMemoriakeor::poner_dato(direccion_mem dir, string id, int valor) {
	int z = dir;
	eliminarFlecha(id);
	while(z!=NULO){
		if(mem[z].id == id){
...
int CSMemoriakeor::obtenerDato(direccion_mem dir, string id) {
	int z = dir;
	bool ex = false;
	eliminarFlecha(id);
	while(z!=NULO){
...
int CSMemoriakeor::Espacio_disponible() {
	int x = libre;
	short c = 0;
	while(x!=NULO){
		c++;
...
int CSMemoriakeor::Espacio_ocupado() {
	short c = (MAX_MEMORIA)-Espacio_disponible();
	return c;
}

bool CSMemoriakeor::dir_libre(direccion_mem dir) {
	int x = libre;
	int c =  false;
...
void CSMemoriakeor::mostrar_memoria(){
	cout<<"+";
